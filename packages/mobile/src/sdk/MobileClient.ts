 ...
 * - devnetUrl: http://10.0.2.2:3999/ (Android emulator compatible)
 */
export class MobileClient extends BaseClient {
  constructor(
    ...
    const dataManager = storageManager
      ? storageManager
      : StorageFactory.getInstance()
    const stacksClient = new StacksClient(
      network,
      stacksConfig?.mainnetUrl || STACKS_API_BASE_URL,
      stacksConfig?.testnetUrl || STACKS_TESTNET_API_BASE_URL,
      stacksConfig?.devnetUrl || STACKS_DEVNET_API_BASE_URL
    )
    const stackingClient = new StackingClient(network)
    super(
